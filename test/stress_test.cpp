 ...
 * @LastEditTime: 2024-05-02 14:28:17
 * @FilePath: /MySkipList/test/stress_test.cpp
 * @Description: 使用多线程实现压力测试
 */
#include <iostream>
#include <chrono> 
// #include <cstdlib>
...
void TestInsertElement(int tid, std::mt19937 &gen) {
  ...
  for (int i = tid*count_each_thread, count = 0; count < count_each_thread; i++) {
    count++;
    test_skip_list.insert_element(GetRandomNumber(gen) % TEST_COUNT, "rain"); 
  }
}
